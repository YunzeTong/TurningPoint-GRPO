...
from flow_grpo.diffusers_patch.sd3_pipeline_with_logprob_fast_my import pipeline_ode_sampling, pipeline_mixed_sampling_with_intermediate_ode
...
def main(_):
    ...
    if accelerator.is_main_process:
        ...
        tensorboard_writer = SummaryWriter(
            ...
            comment=f"sd35_fast_{config.run_name}"
        )
    ...
    while True:
        ...
        for i in tqdm(
            ...
            latents = torch.stack(main_results['latents'], dim=1)  # (batch_size, num_steps, 16, h, w)
            next_latents = torch.stack(main_results['next_latents'], dim=1)  # (batch_size, num_steps, 16, h, w)
        ...
        for sample in samples:
            ...
            # Compute per-timestep rewards with bonus mechanism (OP3 logic)
            timestep_rewards = []
            B_list = []
            main_list = []
            indicator_list = []
        ...
        if accelerator.is_local_main_process and tensorboard_writer and len(samples) > 0:
            ...
            all_process_rewards = torch.cat([s["process_rewards"] for s in samples], dim=0)
            ...
            for t in range(1, num_timesteps):
                # Check how often bonus condition is triggered: s_{t-1} < 0 and s_t > 0
                if all_s_list.shape[1] > t:
                    s_last = all_s_list[:, t-1]
                    s_t = all_s_list[:, t]
                    trigger_ratio = ((s_last < 0).float() * (s_t > 0).float()).mean().item()
        ...
        for k in samples[0].keys():
            if k in special_keys:
                # Keep as list for potential visualization
        ...
        if accelerator.is_main_process and tensorboard_writer:
            ...
            for key, value in gathered_rewards.items():
                if '_strict_accuracy' not in key and '_accuracy' not in key:
                    tensorboard_writer.add_scalar(f"train/reward_{key}", value.mean(), global_step)
