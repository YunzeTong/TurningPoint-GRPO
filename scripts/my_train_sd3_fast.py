...
def eval(pipeline, test_dataloader, text_encoders, tokenizers, config, accelerator, global_step, reward_fn, executor, autocast, num_train_timesteps, ema, transformer_trainable_parameters, tensorboard_writer=None):
    ...
    for test_batch in tqdm(
        ...
        print("check", prompt_embeds.dtype, pooled_prompt_embeds.dtype)
...
def main(_):
    ...
    while True:
        ...
        # Note: In this version without process reward, all timesteps have the same advantage
        if accelerator.is_main_process and tensorboard_writer:
            # advantages shape: (N, T) where all T values are identical for each sample
            ...
            # Log per-timestep for consistency with op3 version (though they're all the same)
                    ...
                    with accelerator.accumulate(transformer):
                        ...
                        if config.train.beta > 0:
                            ...
                            if torch.isnan(kl_loss) or torch.isinf(kl_loss):
                                print("NaN/Inf in kl_loss", kl_loss, epoch, inner_epoch, i, j)
                                print(torch.mean(std_dev_t))
                                raise RuntimeError("NaN/Inf in kl_loss")
