...
def load_geneval(device):
    ...
    def load_models(device):
        ...
        #     )
        CONFIG_PATH = "/mnt/workspace/tyz/A_MODELS/render_text_git/mmdetection/configs/mask2former/mask2former_swin-s-p4-w7-224_lsj_8x2_50e_coco.py"
        # print(CONFIG_PATH)
        OBJECT_DETECTOR = "mask2former_swin-s-p4-w7-224_lsj_8x2_50e_coco"
        CKPT_PATH = os.path.join("/mnt/workspace/tyz/A_MODELS/render_text_git/model/mask2former2", f"{OBJECT_DETECTOR}.pth")
        object_detector = init_detector(CONFIG_PATH, CKPT_PATH, device=device)
        clip_arch =  "ViT-L-14"
        tokenizer = open_clip.get_tokenizer(clip_arch)
        clip_model, _, transform = open_clip.create_model_and_transforms(clip_arch, pretrained="/mnt/workspace/tyz/A_MODELS/open_clip/open_clip_pytorch_model.bin", device=device)
        with open(os.path.join("/mnt/workspace/tyz/A_MODELS/open_clip", "object_names.txt")) as cls_file:
    ...
    object_detector, (clip_model, transform, tokenizer), classnames = load_models(device)
    THRESHOLD = 0.3
