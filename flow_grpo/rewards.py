...
def image_similarity_score(device):
    ...
    scorer = ClipScorer(device=device) # .cuda() # 11.5 correct
...
def ocr_score(device):
    from flow_grpo.ocr_mine import OcrScorer
...
def deqa_score_local(device):
    """Locally use DeQA to compute reward.
    """

    ############## MY CODE #####################

    from flow_grpo.deqa.src.evaluate.scorer import Scorer
    model = Scorer(device=device)
    ############# MY CODE ######################
...
def geneval_score_local(device):
    """Local GenEval scoring using object detection and CLIP models.
    """
    from flow_grpo.geneval_utils import load_geneval
    
    # Load local GenEval evaluator
    compute_geneval = load_geneval(device)
    
    def _fn(images, prompts, metadatas, only_strict=True):
        del prompts  # Not used in GenEval
        ...
        # Compute GenEval scores using local models
        scores, rewards, strict_rewards, group_rewards, group_strict_rewards = compute_geneval(
            pil_images, metadatas, only_strict=only_strict
        )
        
        # Convert lists to numpy arrays to match other scoring functions
        scores = np.array(scores, dtype=np.float32)
        rewards = np.array(rewards, dtype=np.float32)
        strict_rewards = np.array(strict_rewards, dtype=np.float32)
        
        # Convert grouped rewards dictionaries - values should be numpy arrays
        group_rewards_np = {k: np.array(v, dtype=np.float32) for k, v in group_rewards.items()}
        group_strict_rewards_np = {k: np.array(v, dtype=np.float32) for k, v in group_strict_rewards.items()}
        
        return scores, rewards, strict_rewards, group_rewards_np, group_strict_rewards_np
...
def unifiedreward_score_local(device):
    """
    Local UnifiedReward scoring using the model loaded directly.
    No need for sglang server.
    """
    from flow_grpo.unifiedreward_scorer import UnifiedRewardScorer

    scorer = UnifiedRewardScorer(
        model_path="/mnt/workspace/tyz/A_MODELS/UnifiedReward-qwen-7b",
        device=device,
    ...
    def _fn(images, prompts, metadata):
        ...
        # Convert to PIL Images and resize to 512x512 (consistent with sglang version)
...
def multi_score(device, score_dict):
    score_functions = {
        ...
        "unifiedreward": unifiedreward_score_local,
        "geneval": geneval_score,
        "geneval_local": geneval_score_local,
    ...
    def _fn(images, prompts, metadata, ref_images=None, only_strict=True):
        ...
        for score_name, weight in score_dict.items():
            if score_name in ["geneval", "geneval_local"]:
