...
class MPLUGOwl2LlamaModel(MPLUGOwl2MetaModel, LlamaModel):
    ...
    def __init__(self, config: MPLUGOwl2Config):
        ...
        # Initialize attention implementation flags
        # These are needed by the model_forward function in modeling_llama2.py
        if hasattr(config, '_attn_implementation'):
            self._use_flash_attention_2 = config._attn_implementation == "flash_attention_2"
            self._use_sdpa = config._attn_implementation == "sdpa"
        else:
            # Default to standard attention if not specified
