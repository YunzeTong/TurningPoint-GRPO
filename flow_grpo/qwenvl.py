...
class QwenVLScorer(torch.nn.Module):
    def __init__(self, device="cuda", dtype=torch.bfloat16):
        ...
        self.model = Qwen2_5_VLForConditionalGeneration.from_pretrained(
            "/mnt/workspace/wanggui.hwg/checkpoints/qwen_lab/Qwen2.5-VL-7B-Instruct",
            torch_dtype=self.dtype,
            attn_implementation="flash_attention_2",
            device_map=None,
        ).to(self.device)
        self.model.requires_grad_(False)
        self.processor = AutoProcessor.from_pretrained("/mnt/workspace/wanggui.hwg/checkpoints/qwen_lab/Qwen2.5-VL-7B-Instruct", use_fast=True)
        self.task = '''
